// Declare a custom data structure with specific alignment
struct MyStruct
    int a;
    double b;
...
// 'register' is a keyword that suggests to the compiler to store a variable in a CPU register for faster access.
...
// 'union' is used to define a composite data type that can hold different types.
union Data {
    int intValue;
    float floatValue;
    char stringValue[20];
...
// `_Noreturn` indicates that the function will not return.
...
// _Static_assert is used to check a compile-time condition.
...
// _Thread_local is used to declare thread-local variables.
